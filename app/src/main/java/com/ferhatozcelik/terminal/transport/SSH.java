...
import com.ferhatozcelik.terminal.bean.PubkeyBean;
...
import com.ferhatozcelik.terminal.util.PubkeyUtils;
...
public class SSH extends AbsTransport implements ConnectionMonitor, InteractiveCallback, AuthAgentCallback {
	...
	private boolean tryAutoAuthentication = false;
	...
	private void authenticate() {
		...
		try {
			...
			if (!pubkeysExhausted && pubkeyId != HostDatabase.PUBKEYID_NEVER && connection.isAuthMethodAvailable(host.getUsername(), AUTH_PUBLICKEY)) {
				...
				if (pubkeyId == HostDatabase.PUBKEYID_NEVER) {
					// try each of the in-memory keys
					bridge.outputLine(manager.res
							.getString(R.string.terminal_auth_pubkey_any));
					for (Entry<String, TerminalManager.KeyHolder> entry : manager.loadedKeypairs.entrySet()) {
	...
	public void connect() {
		...
		try {
			...
			*/
			...
			//bridge.outputLine(manager.res.getString(R.string.terminal_kex_algorithm, connectionInfo.keyExchangeAlgorithm));

			if (connectionInfo.clientToServerCryptoAlgorithm.equals(connectionInfo.serverToClientCryptoAlgorithm) && connectionInfo.clientToServerMACAlgorithm.equals(connectionInfo.serverToClientMACAlgorithm)) {
				//bridge.outputLine(manager.res.getString(R.string.terminal_using_algorithm, connectionInfo.clientToServerCryptoAlgorithm, connectionInfo.clientToServerMACAlgorithm));
			} else {
				//bridge.outputLine(manager.res.getString(R.string.terminal_using_c2s_algorithm, connectionInfo.clientToServerCryptoAlgorithm, connectionInfo.clientToServerMACAlgorithm));
				//bridge.outputLine(manager.res.getString(R.string.terminal_using_s2c_algorithm, connectionInfo.serverToClientCryptoAlgorithm, connectionInfo.serverToClientMACAlgorithm));
			}
		} catch (IOException e) {
